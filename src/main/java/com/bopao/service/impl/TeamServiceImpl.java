...
//        }
    ...
    public boolean updateTeam(TeamUpdateRequest teamUpdateRequest, User loginUser) {
        ...
        // 如果是公开或者私有 将密码设置为null
        if (TeamStatusEnum.PUBLIC.equals(enumByValue) || TeamStatusEnum.PRIVATE.equals(enumByValue)) {
            password = null;
        }
        //如果status为公开的不允许设置密码
...
//            queryWrapper.eq("status", statusEnum.getValue());
        ...
        Map<Long, Integer> teamUserCountMap = new HashMap<>();
        // 关联查询创建人的用户信息
        for (Team team : teamList) {
            Long teamId = team.getId();
            int haveJoinNum = countIntTeamUserByTeamId(teamId);
            teamUserCountMap.put(teamId, haveJoinNum);
            Long userId = team.getUserId();
            if (userId == null) {
                continue;
            }
            ...
            // 设置已加入人数
            int hasJoinNum = teamUserCountMap.get(teamId) != null ? teamUserCountMap.get(teamId).intValue() : 0;
...
//    }
    ...
    private int countIntTeamUserByTeamId(long teamId) {
        QueryWrapper<UserTeam> userTeamQueryWrapper = new QueryWrapper<>();
        userTeamQueryWrapper.eq("teamId", teamId);
        long count = userTeamService.count(userTeamQueryWrapper);
        if (count > Integer.MAX_VALUE || count < Integer.MIN_VALUE) {
            throw new IllegalArgumentException("Count value is out of range for int: " + count);
        }
